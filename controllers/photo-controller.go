...
func (c *photoController) All(context *gin.Context) {
	...
	res := helpers.BuildResponse(true, "success", photos)
...
func (c *photoController) FindByID(context *gin.Context) {
	...
	if (photo == app.Photo{}) {
		...
		res := helpers.BuildResponse(true, "success", photo)
		context.JSON(http.StatusOK, res)
	}
}
...
func (c *photoController) Insert(context *gin.Context) {
	...
	if errModel != nil {
		...
		response := helpers.BuildResponse(true, "success", result)
...
func (c *photoController) Update(context *gin.Context) {
	...
	if c.photoService.IsAllowedToEdit(userID, photoUpdateModel.ID) {
		...
		response := helpers.BuildErrorResponse("You dont have permission", "You're not the owner", helpers.EmptyObj{})
...
func (c *photoController) Delete(context *gin.Context) {
	...
	if c.photoService.IsAllowedToEdit(userID, photo.ID) {
		...
		response := helpers.BuildErrorResponse("You don't have permission", "You're not the owner", helpers.EmptyObj{})
		context.JSON(http.StatusForbidden, response)
	}
}
